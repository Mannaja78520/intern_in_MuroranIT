...
class VideoAnalyzer(PipelineUIMixin, FlowchartMixin, UserPipelinesMixin):
    ...
    def _process(self, f_rgb, f_ir):
        ...
        def _make_composite(operands, overlay_bgr,
            ...
            Returns (composite_image, tile_images) where tile_images
            is the per-tile BGR list so callers can publish each as
            its own view for individual zoom."""
            tiles = []
            tile_images = []
            for i, op in enumerate(operands):
                if i:
                    tiles.append(_glyph("+", tile_h))
                _t = cv2.resize(_to_bgr(op), (tile_w, tile_h))
                tiles.append(_t)
                tile_images.append(_t)
            tiles.append(_glyph("->", tile_h))
            _res = cv2.resize(_to_bgr(overlay_bgr), (tile_w, tile_h))
            tiles.append(_res)
            tile_images.append(_res)
