...
class FlowchartMixin:
    ...
    def _open_all_masks_window(self):
        ...
        # -- Node lists ------------------------------------------------
        # HSV per-channel grayscale tiles (row 0) and their matching
        # colormap tiles (row _rgb_after_pm) are kept as 6 individual
        # nodes — clearer than one merged "pair" tile because each can
        # be zoomed / pixel-hovered on its own.
        rgb_nodes = [
            ("rgb_raw",        0, 0, " 1  Input"),
            ("rgb_blur",       1, 0, " 2  Blur*"),
            ...
            *[(f"rgb_pre_step{i+1}",
            ...
            ("rgb_hsv_H_cmap", 3, _rgb_after_pm,     " 4c Hue cmap"),
            ("rgb_hsv_S_cmap", 4, _rgb_after_pm,     " 5c Sat cmap"),
            ("rgb_hsv_V_cmap", 5, _rgb_after_pm,     " 6c Val cmap"),
            ("rgb_m1",        2, _rgb_after_pm + 1, " 7  Hue mask 1"),
            ("rgb_m2",        3, _rgb_after_pm + 1, " 8  Hue mask 2"),
            ("rgb_hsv_mask",  4, _rgb_after_pm + 1, " 9  HSV mask"),
            ("rgb_bgsub",     5, _rgb_after_pm + 1, "10  BG sub"),
            ("rgb_mask_pre",  6, _rgb_after_pm + 1, "11  Pre-pipeline"),
            *[(f"rgb_step{i+1}",
               rgb_positions[i][0],
               rgb_positions[i][1],
               f"S{i+1}: ?")
        ...
        for up in user_pipes:
            ...
            # Info nodes wrap to a NEW row whenever the current row
            # would overflow STEPS_PER_ROW columns — otherwise a
            # multi-channel HSV branch with bgsub puts ~10 tiles in a
            # single row and the layout becomes unreadable.
            _info_nodes = []
            _info_max_row = 0
            try:
                ...
                # Cursor state for wrapping. row 0 col 0 holds the
                # source `_input_id`, so info nodes start at col 1
                # on row 0; subsequent wrapped rows can use col 0.
                _ic_state = {"col": 1, "row": 0}
                def _push_info(suffix, label):
                    if _ic_state["col"] >= STEPS_PER_ROW:
                        _ic_state["row"] += 1
                        _ic_state["col"] = 0
                    _info_nodes.append(
                        (f"_info_{_nm}_{suffix}",
                         _ic_state["col"], _ic_state["row"], label))
                    _ic_state["col"] += 1

                # Source-image pipeline.
                if _btype in ("rgb", "ir"):
                    _push_info("src_raw",   "src raw")
                    if _pre_k > 0:
                        _push_info("src_blur",  "src blur")
                    if _show_bg:
                        _push_info("src_bgsub", "src bg-sub")
                if _btype == "rgb":
                    if "H" in _ch_mode or _ch_mode == "full":
                        _push_info("h_src",  "H channel")
                        _push_info("h_mask", "H mask (det)")
                    if "S" in _ch_mode or _ch_mode == "full":
                        _push_info("s_src",  "S channel")
                        _push_info("s_mask", "S mask (det)")
                    if "V" in _ch_mode or _ch_mode == "full":
                        _push_info("v_src",  "V channel")
                        _push_info("v_mask", "V mask (det)")
                elif _btype == "ir":
                    _push_info("ir_src",  "IR frame")
                    _push_info("ir_mask", "IR mask (det)")
                if _show_bg:
                    _push_info("bgsub", "BG-sub FG")
                # Combined detection thumbnail (always last).
                _push_info("det", "branch_det")
                _info_max_row = _ic_state["row"]
            except Exception:
                _info_nodes = []
                _info_max_row = 0
            ...
            edges_up = []
            # Source -> first PM step (or first morph step if no PM).
            if _branch_pre_n > 0:
                edges_up.append((_input_id, f"up_{_nm}_pre_step1"))
                for i in range(_branch_pre_n - 1):
                    edges_up.append((f"up_{_nm}_pre_step{i+1}",
                                     f"up_{_nm}_pre_step{i+2}"))
                # Hand off into the morph chain (or straight to output
                # when the branch has no morph steps).
                _post_pm_dst = (f"up_{_nm}_step1" if _n > 0 else _output_id)
