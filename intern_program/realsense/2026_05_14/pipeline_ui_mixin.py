...
class PipelineUIMixin:
    ...
    # Build / rebuild
    # ------------------------------------------------------------------
    # How many step cards before we wrap to a new row. 3-per-row gives
    # each card more horizontal breathing room so the dynamic param
    # labels (e.g. "Kernel size (odd)", "Block size") don't get
    # squeezed into the spinboxes.
    STEPS_PER_UI_ROW = 3

    def _grid_step_card(self, sf, i):
        """Place an existing step card `sf` at grid cell
        (i // STEPS_PER_UI_ROW, i % STEPS_PER_UI_ROW)."""
