...
# Default Tk colours on Linux are light-gray-on-near-black: gray text
# against gray panels is unreadable. The theme below pins a dark
# background everywhere and forces high-contrast labels.
UI_BG          = "#1f1f2b"   # near-black panel background
UI_BG_PANEL    = "#28283a"   # slightly lighter for panels/inputs
UI_BG_INPUT    = "#33334a"   # input fields
UI_FG          = "#f0f0f0"   # primary text — near white
UI_FG_MUTED    = "#c8c8c8"   # secondary text (already used in app)
UI_FG_ACCENT   = "#aaccff"   # accent (blue) for headings
UI_FONT_FAMILY = "DejaVu Sans"  # friendly, well-hinted on Linux


def _apply_ui_theme(root):
    """Install a dark theme via option_add + ttk.Style so every
    tk and ttk widget renders with consistent, high-contrast colours.
    Called once from VideoAnalyzer.__init__."""
    try:
...
class VideoAnalyzer(PipelineUIMixin, FlowchartMixin, UserPipelinesMixin):
    ...
    def _process(self, f_rgb, f_ir):
        ...
        # Paired views: channel grayscale on top, colormap below — one
        # thumbnail per HSV channel instead of two side-by-side tiles.
        # Halving each half keeps the combined aspect == source aspect,
        # so the All-Masks thumbnail resize doesn't squish it.
        ...
        if self.all_masks_win and self.all_masks_win.winfo_exists():
            ...
            # so a tiny dedicated formatter keeps the label short.
            def _pm_lbl(i, en, op_v):
                if not en.get():
                    return f"PM{i+1}: -"
                return f"PM{i+1}: {op_v.get()}"
            ...
            for _i, (_en, _op, _n, _d, _kx, _ky, _t, _cen, _cop, _csr) in enumerate(self.ir_pipeline):
                ...
                if _k in self.all_masks_step_labels:
                    self.all_masks_step_labels[_k].config(
                        text=_step_lbl(_i, _en, _op, _cen, _cop, _csr))
            for _i, (_en, _op, *_rest) in enumerate(self.rgb_pre_pipeline):
                _k = f"rgb_pre_step{_i+1}"
                if _k in self.all_masks_step_labels:
                    self.all_masks_step_labels[_k].config(
                        text=_pm_lbl(_i, _en, _op))
            for _i, (_en, _op, *_rest) in enumerate(self.ir_pre_pipeline):
            ...
            for _up in self.user_pipelines:
                ...
                for _i, (_en, _op, _n, _d, _kx, _ky, _t,
                    ...
                    if _k in self.all_masks_step_labels:
                        self.all_masks_step_labels[_k].config(
                            text=_step_lbl(_i, _en, _op, _cen, _cop, _csr))
                for _i, (_en, _op, *_rest) in enumerate(_up.get("pre_steps") or []):
