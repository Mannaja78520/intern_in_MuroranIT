...
from pipeline_ui_mixin    import PipelineUIMixin, OVERLAY_COLORS
...
class VideoAnalyzer(ProcessingMixin, ConfigIOMixin, ExportMixin,
    ...
    def _build_ui(self):
        ...
        _file_menu.add_command(label="Quick save config        Ctrl+S",
                               command=self._quick_save_config)
        _file_menu.add_command(label="Quick load config        Ctrl+O",
                               command=self._quick_load_config)
        _file_menu.add_separator()
        _file_menu.add_command(label="Save config as...",
                               command=self._save_config_dialog)
        _file_menu.add_command(label="Load config from...",
                               command=self._load_config_dialog)
        _file_menu.add_separator()
        _file_menu.add_command(label="Export pipeline  (choose outputs)...",
        ...
        # Fixed character width + left anchor so a long path in a
        # status message can never widen `sel` (and the whole window,
        # which shifts the 6 video panels). Long paths are also
        # ellipsized by _short_path() before being shown here.
        self.lbl_status = tk.Label(sel, text="- select a folder and click Load -",
                                   fg="#c8c8c8", width=64, anchor="w")
        ...
        # LAB L/a*/b* rows — used by the 'LAB' and 'HSV hue + LAB'
        # modes. a* is red<->green (128 neutral; >128 = red), b* is
        # yellow<->blue. For red: raise A_min (~150). L is lightness
        # (0=black .. 255=white); it defaults to the full 0..255 range
        # so brightness is ignored unless you deliberately narrow it
        # (e.g. to reject very dark or very bright pixels).
        _llab_lbl = _sec_lbl("-- LAB  L (lightness, 0..255) --")
        _lmin_row = add_param(rgb_tab, "L_min",   0, 0, 255)
        _lmax_row = add_param(rgb_tab, "L_max", 255, 0, 255)
        ...
        self._rgb_detect_widgets = [
            ...
            (_llab_lbl, _lbl_kw, _LABM), (_lmin_row, _row_kw, _LABM),
            (_lmax_row, _row_kw, _LABM),
        ...
        # YOLO box size tuning — applies to BOTH the main YOLO and the
        # per-step YOLO add-on. Scale = % of the detected box around its
        # centre (100 = unchanged, >100 bigger, <100 smaller); Pad adds
        # pixels on every side (negative trims). Use bigger to leave a
        # margin to subtract; smaller to focus tightly inside the box.
        tk.Label(shared_tab,
                 text="-- YOLO box size (scale % + pixel pad) --",
                 font=("DejaVu Sans", 9), fg="#c8c8c8").pack(anchor="w")
        add_param(shared_tab, "YOLO_Box_Scale", 100, 20, 300,
                  label="Box scale %")
        add_param(shared_tab, "YOLO_Box_Pad", 0, -200, 200,
                  label="Box pad px")
        ...
        # -- Combine tab content ---------------------------------------
        # A <op> B -> 'combined'. A and B are picked with the cascading
        # From -> Step picker, so either operand can be a RAW image or
        # any pipeline MASK. 'combined' itself is always a binary mask
        # (so it stays usable as an input to other masks / branches);
        # the colour composite is published separately as
        # 'combined_overlay'. Everything defaults to "none" — the
        # Combine tab does nothing until you configure it.
        _hdr(combine_tab,
             "Combine  (A <op> B -> binary 'combined';  raw or mask)",
             "#ffdd66")

        # Operand A — cascading From/Step picker (raw or mask).
        self.combine_a_var = tk.StringVar(value="none")
        fa = tk.LabelFrame(combine_tab, text="A  (operand 1)",
                           font=("DejaVu Sans", 8, "bold"), fg="#4488ff")
        fa.pack(fill="x", padx=4, pady=(3, 1))
        self._make_cascading_picker(
            fa, self.combine_a_var, [],
            label_from="A from:", label_step="A step:",
            allow_none=True, include_this_pipe=False)

        # Op — AND / OR / XOR / OVERLAY / none.  Default none.
        orow = tk.Frame(combine_tab); orow.pack(fill="x", padx=8, pady=2)
        tk.Label(orow, text="Op:", font=("DejaVu Sans", 9, "bold"),
                 width=8, anchor="w").pack(side="left")
        self.combine_op_var = tk.StringVar(value="none")
        ttk.Combobox(orow, textvariable=self.combine_op_var,
                     values=["none", "AND", "OR", "XOR", "OVERLAY"],
                     width=9, state="readonly",
                     font=("DejaVu Sans", 9, "bold")
                     ).pack(side="left", padx=4)

        # Operand B — cascading From/Step picker (raw or mask).
        self.combine_b_var = tk.StringVar(value="none")
        fb = tk.LabelFrame(combine_tab, text="B  (operand 2)",
                           font=("DejaVu Sans", 8, "bold"), fg="#44cc66")
        fb.pack(fill="x", padx=4, pady=(1, 1))
        self._make_cascading_picker(
            fb, self.combine_b_var, [],
            label_from="B from:", label_step="B step:",
            allow_none=True, include_this_pipe=False)

        # Mask colours — used by the 'combined_overlay' colour composite
        # (A painted in colour 1, B in colour 2). They never change the
        # binary 'combined' result.
        colrow = tk.Frame(combine_tab); colrow.pack(fill="x", padx=8, pady=2)
        tk.Label(colrow, text="Colours:", font=("DejaVu Sans", 9, "bold"),
                 width=8, anchor="w").pack(side="left")
        tk.Label(colrow, text="Mask 1 (A):", font=("DejaVu Sans", 8),
                 fg="#4488ff").pack(side="left")
        self.combine_c1_var = tk.StringVar(value="red")
        ttk.Combobox(colrow, textvariable=self.combine_c1_var,
                     values=list(OVERLAY_COLORS.keys()),
                     width=9, state="readonly",
                     font=("DejaVu Sans", 8)).pack(side="left", padx=(2, 8))
        tk.Label(colrow, text="Mask 2 (B):", font=("DejaVu Sans", 8),
                 fg="#44cc66").pack(side="left")
        self.combine_c2_var = tk.StringVar(value="green")
        ttk.Combobox(colrow, textvariable=self.combine_c2_var,
                     values=list(OVERLAY_COLORS.keys()),
                     width=9, state="readonly",
                     font=("DejaVu Sans", 8)).pack(side="left", padx=2)
        tk.Label(combine_tab,
                 text=("-> view 'combined' (binary mask) or "
                       "'combined_overlay' (colour) in any panel / zoom. "
                       "Press Apply (F5) after changing these."),
    ...
    def _apply_rgb_mode_ui(self, *_):
        ...
        _descs = {
            ...
            "lab": ("LAB L/a*/b*  -  HSV ignored. a* is red<->green "
                    "(>128 = red), b* is yellow<->blue. White / gray / "
                    "black all sit near a*=b*=128, so a coloured object "
                    "is found on ANY-brightness background. For red, "
                    "raise A_min (~150+). L (lightness) defaults to "
                    "0..255 = ignored; narrow it to also drop too-dark "
                    "or too-bright pixels."),
            "hsv_lab": ("HSV + LAB  -  runs the Hue detector AND the "
                        "LAB L/a*/b* detector and keeps only pixels in "
    ...
    def _bind_keys(self):
        ...
        # Ctrl+S / Ctrl+O = quick save / load config (no dialog).
        self.root.bind_all("<Control-s>", self._quick_save_config)
        self.root.bind_all("<Control-o>", self._quick_load_config)
    ...
    def _start_record(self):
        ...
        # 7th video: all six panels stacked into one 2x3 mosaic
        # (top row = RGB raw/det/mask, bottom row = IR raw/det/mask),
        # matching the on-screen layout — review the whole analysis
        # as a single file.
        self.writers["combined"] = cv2.VideoWriter(
            os.path.join(out_dir, "combined_6up.mp4"),
            fourcc, fps, (sz[0] * 3, sz[1] * 2))
        # Snapshot the current pipeline config next to the videos so
        # the recording is self-describing and exactly reproducible.
        try:
            import json
            with open(os.path.join(out_dir, "analyzer_config.json"),
                      "w") as _cf:
                json.dump(self._collect_config(), _cf, indent=2)
        except Exception:
            pass
    ...
    def _stop_record(self):
        ...
        self.lbl_rec_status.config(
            text=f"Saved -> {self.rec_dir}/  "
                 f"(6 panels + combined_6up.mp4 + analyzer_config.json)")
