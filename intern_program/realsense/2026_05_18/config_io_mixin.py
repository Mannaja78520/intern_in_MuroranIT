...
class ConfigIOMixin:
    ...
    def _collect_config(self):
        ...
        cfg = {
            ...
            "strings": {
                ...
                "combine_a":  getattr(self, "combine_a_var",
                                      tk.StringVar(value="none")).get(),
                "combine_b":  getattr(self, "combine_b_var",
                                      tk.StringVar(value="none")).get(),
                "combine_op": getattr(self, "combine_op_var",
                                      tk.StringVar(value="none")).get(),
                "combine_c1": getattr(self, "combine_c1_var",
                                      tk.StringVar(value="red")).get(),
                "combine_c2": getattr(self, "combine_c2_var",
                                      tk.StringVar(value="green")).get(),
            },
    ...
    def _apply_config_inner(self, cfg):
        ...
        # Backward-compat: the LAB detect-mode dropdown label gained an
        # explicit "L" when L (lightness) became adjustable. Configs
        # saved before that change carry the old label — remap it so
        # the mode still resolves instead of falling back to "hue".
        if strs.get("rgb_detect_mode") == "LAB a*b* range":
            strs = dict(strs)
            strs["rgb_detect_mode"] = "LAB L/a*/b* range"
        for src_key, attr in [
            ...
            ("combine_a",  "combine_a_var"),
            ("combine_b",  "combine_b_var"),
            ("combine_op", "combine_op_var"),
            ("combine_c1", "combine_c1_var"),
            ("combine_c2", "combine_c2_var"),
        ]:
    ...
    def _last_config_dir_name(self):
        """(initialdir, initialfile) seeded from the last config path so
        every Save/Load dialog reopens where the user last worked."""
        last = self._load_pref("last_config_path", "")
        if last:
            return os.path.dirname(last), os.path.basename(last)
    ...
    def _save_config_dialog(self):
        ...
        import json
        initdir, initfile = self._last_config_dir_name()
        ...
        try:
            ...
            self.lbl_status.config(
                text="Config saved -> %s  (Ctrl+S to re-save here)"
    ...
    def _load_config_dialog(self):
        ...
        try:
            ...
            self._save_pref("last_config_path", path)
            self.lbl_status.config(
                text="Config loaded <- %s  (Ctrl+O to reload)"
                     % self._short_path(path),
                fg="#88ff88")
        except Exception as e:
            self.lbl_status.config(
    ...
    # ------------------------------------------------------------------
    # Quick save / load — one keypress, no dialog. Reuses the last
    # config path so re-tuning is fast; the very first time (no path
    # remembered yet) it falls back to the full dialog.
    # ------------------------------------------------------------------
    def _default_config_path(self):
        """Where Quick Save writes when nothing was saved/loaded yet:
        analyzer_config.json next to this program."""
        try:
            here = os.path.dirname(os.path.abspath(__file__))
        except Exception:
            here = os.getcwd()
        return os.path.join(here, "analyzer_config.json")

    @staticmethod
    def _short_path(path, maxlen=50):
        """Ellipsize a long path for status-bar display so the status
        label (and hence the window) never has to grow to fit it.
        Keeps the tail (filename + parent dir), which is the useful part."""
        path = str(path)
        if len(path) <= maxlen:
            return path
        return "..." + path[-(maxlen - 3):]

    def _quick_save_config(self, event=None):
        """Save straight to the last-used config file (no dialog)."""
        import json
        last = self._load_pref("last_config_path", "")
        if not last:
            # Nothing remembered yet — let the user pick a place once.
        ...
        try:
            with open(last, "w") as f:
                json.dump(self._collect_config(), f, indent=2)
            self.lbl_status.config(
                text="Quick-saved -> %s" % self._short_path(last),
                fg="#88ff88")
        except Exception as e:
            self.lbl_status.config(
                text="Quick save failed: %s" % e, fg="red")
        return "break"

    def _quick_load_config(self, event=None):
        """Load straight from the last-used config file (no dialog).
        Opens the picker instead if nothing has been loaded yet."""
        import json
        last = self._load_pref("last_config_path", "")
        ...
        try:
            ...
            self._apply_config(cfg)
            self.lbl_status.config(
                text="Quick-loaded <- %s" % self._short_path(last),
                fg="#88ff88")
        except Exception as e:
            self.lbl_status.config(
                text="Quick load failed: %s" % e, fg="red")
        return "break"
