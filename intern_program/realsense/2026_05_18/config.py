...
# -- RGB detection modes ---------------------------------------------------
# How the RGB binary detection mask is built from the HSV image.
#   "hue"     - match the two Hue ranges (H1/H2) plus the S and V
#               bands. Finds COLOURED objects (e.g. a red cable).
#               Fails on white / gray / black: those are desaturated
#               so their Hue is undefined / noisy and nothing matches.
#   "achroma" - ignore Hue; keep pixels whose Saturation AND Value are
#               inside their bands. Set S low (e.g. 0..40) to isolate
#               white / gray / black objects. This is the mode for
#               desaturated targets.
#   "value"   - ignore Hue AND Saturation; keep pixels in the V band
#               only. Pure brightness segmentation.
# Maps the UI dropdown label -> internal mode key. Key insertion order
# is the order shown in the dropdown.
#   "lab"     - ignore HSV; threshold the LAB L / a* / b* channels. a*
#               is the red<->green axis, b* the yellow<->blue axis;
#               white, gray and black ALL sit near a*=b*=128, so a
#               coloured object pops out regardless of how bright/dark
#               the background is. Best for "red on white AND on black".
#               L (lightness) defaults to the full 0..255 range
#               (ignored) but can be narrowed to also reject too-dark
#               or too-bright pixels.
#   "hsv_lab" - run BOTH the HSV hue detector and the LAB L/a*/b*
#               detector and AND the two masks. Most selective: a
#               pixel must match the hue range AND the LAB colour box,
#               which rejects a bright white floor that leaks past the
#               hue test alone.
RGB_DETECT_MODES = {
    "Color (HSV hue)":         "hue",
    "Achromatic (white/gray)": "achroma",
    "Brightness (V only)":     "value",
    "LAB L/a*/b* range":       "lab",
    "HSV hue + LAB (red)":     "hsv_lab",
}

# Short tag burnt into the detection-mask thumbnail so the panel /
# flowchart view shows at a glance which mode produced it.
RGB_DETECT_TAGS = {
    "hue":     "HUE",
    "achroma": "ACHROMATIC",
    "value":   "BRIGHTNESS",
