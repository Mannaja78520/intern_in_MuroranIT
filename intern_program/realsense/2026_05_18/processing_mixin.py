...
from config import (
    ...
    FC_W, FC_H,
    is_pre_morph_op,
    RGB_DETECT_MODES, RGB_DETECT_TAGS,
)
...
class ProcessingMixin:
    ...
    def _yolo_box_adjust(self, x1, y1, x2, y2, w, h,
                         scale_pct=None, pad_px=None):
        """Grow / shrink a raw YOLO box before it is drawn or filled
        into a mask. By default 'YOLO_Box_Scale' scales it (%) around
        its centre and 'YOLO_Box_Pad' adds pixels on every side
        (negative = trim). A per-step YOLO card may pass `scale_pct` /
        `pad_px` to override the shared values for that step only.
        Result is clamped to the WxH image. Use it bigger to leave a
        margin to subtract, or smaller to focus tightly inside the box."""
        if scale_pct is None:
            try:
                scale = self.sv["YOLO_Box_Scale"].get() / 100.0
            except Exception:
                scale = 1.0
        else:
        ...
        if pad_px is None:
            try:
                pad = int(self.sv["YOLO_Box_Pad"].get())
            except Exception:
                pad = 0
        else:
            pad = int(pad_px)
    ...
    # -- RGB detection mode ------------------------------------------------
    def _rgb_detect_key(self):
        """Internal mode key ('hue' / 'achroma' / 'value') for the
        current RGB detection-mode selection. Falls back to 'hue' when
        the var is missing (e.g. an old config without the dropdown)."""
        try:
            return RGB_DETECT_MODES.get(self.rgb_detect_mode.get(), "hue")
        except Exception:
            return "hue"

    # -- BG-sub 'auto' source resolution ----------------------------------
    def _bgsub_auto_rgb(self):
        """Resolve the 'auto' BG-sub source for the main RGB side to a
        concrete feed that matches the active RGB detection mode, so
        the background model is built on the SAME signal the detector
        thresholds:
          hue / hsv_lab -> full BGR frame   (colour detection)
          achroma / value -> V channel      (brightness segmentation)
          lab            -> a* channel      (LAB colour axis)"""
        return {"hue": "rgb", "hsv_lab": "rgb",
                "achroma": "V", "value": "V",
                "lab": "a"}.get(self._rgb_detect_key(), "rgb")

    def _bgsub_auto_branch(self, up):
        """Resolve the 'auto' BG-sub source for a branch. IR branches
        feed the IR frame; RGB/custom branches pick the most relevant
        single channel from the branch's CHANNELS mode."""
        try:
            if up.get("type") and up["type"].get() == "ir":
                return "ir"
            ch = up["channels"].get() if up.get("channels") else "HSV"
        except Exception:
            return "rgb"
        for c in ("V", "a", "S", "b", "H"):
            if c in ch:
    ...
    def _lab_mask(self, lab):
        """LAB L/a*/b* threshold mask. a* is red<->green, b* is
        yellow<->blue (both 128 = neutral); L is lightness. Reads
        L_min/L_max/A_min/A_max/B_min/B_max from the detection
        sliders. White / gray / black sit at a*=b*=128, so banding
        a*/b* away from 128 isolates a coloured object on any
        brightness; L defaults to 0..255 (ignored) but can be
        narrowed to also reject too-dark or too-bright pixels."""
        try:
            lmn = self.sv["L_min"].get(); lmx = self.sv["L_max"].get()
            amn = self.sv["A_min"].get(); amx = self.sv["A_max"].get()
            bmn = self.sv["B_min"].get(); bmx = self.sv["B_max"].get()
        except Exception:
            lmn, lmx, amn, amx, bmn, bmx = 0, 255, 150, 255, 0, 255
        return cv2.inRange(lab, (lmn, amn, bmn), (lmx, amx, bmx))

    def _rgb_detect(self, hsv, lab, h1l, h1h, h2l, h2h,
                    smin, smax, vmin, vmax):
        """Build the RGB detection masks from the HSV + LAB images
        according to the selected detection mode (config.RGB_DETECT_MODES).

        Returns (m1, m2, mask):
          hue     - m1/m2 are the two Hue-range masks; mask = m1 OR m2.
          achroma - Hue ignored; S+V band only (white/gray/black).
          value   - V band only.
          lab     - m1 is the LAB L/a*/b* mask, m2 empty.
          hsv_lab - m1 = HSV hue mask, m2 = LAB mask, mask = m1 AND m2
                    (most selective — rejects bright background that
                    leaks past hue alone).
        m2 is always returned so the rgb_m1 / rgb_m2 views stay valid."""
        mode = self._rgb_detect_key()
        if mode == "achroma":
            m1 = cv2.inRange(hsv, (0, smin, vmin), (180, smax, vmax))
            return m1, np.zeros_like(m1), m1
        if mode == "value":
            m1 = cv2.inRange(hsv, (0, 0, vmin), (180, 255, vmax))
            return m1, np.zeros_like(m1), m1
        if mode == "lab":
            m1 = self._lab_mask(lab)
            return m1, np.zeros_like(m1), m1
        if mode == "hsv_lab":
            _a = cv2.inRange(hsv, (h1l, smin, vmin), (h1h, smax, vmax))
            _b = cv2.inRange(hsv, (h2l, smin, vmin), (h2h, smax, vmax))
            hue  = cv2.bitwise_or(_a, _b)
            labm = self._lab_mask(lab)
            return hue, labm, cv2.bitwise_and(hue, labm)
        # default: hue
        m1 = cv2.inRange(hsv, (h1l, smin, vmin), (h1h, smax, vmax))
        m2 = cv2.inRange(hsv, (h2l, smin, vmin), (h2h, smax, vmax))
        return m1, m2, cv2.bitwise_or(m1, m2)

    def _stamp_mode_tag(self, mask_gray):
        """Return a display copy of a single-channel detection mask
        with the active mode tag (HUE / ACHROMATIC / BRIGHTNESS) burnt
        into the corner, so the panel and flowchart thumbnail show
        which detection method produced it. The live detection mask
        is never modified — only this display copy is stamped."""
        out = mask_gray.copy()
        try:
            tag = RGB_DETECT_TAGS.get(self._rgb_detect_key(), "HUE")
            # Black outline then white fill so the tag stays legible
            # whatever the mask underneath looks like.
            cv2.putText(out, tag, (4, 16), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, 0,   3, cv2.LINE_AA)
            cv2.putText(out, tag, (4, 16), cv2.FONT_HERSHEY_SIMPLEX,
    ...
    def _process(self, f_rgb, f_ir):
        ...
        # RGB detection — placeholder values; real detection runs
        # after the pre-morph stage modifies `work_rgb`. The detection
        # mode (hue / achromatic / brightness / lab / hsv_lab) decides
        # how the mask is formed — see _rgb_detect. LAB is computed
        # alongside HSV so the LAB modes and the BG-sub L/a/b feeds can
        # use it.
        hsv = cv2.cvtColor(work_rgb, cv2.COLOR_BGR2HSV)
        lab = cv2.cvtColor(work_rgb, cv2.COLOR_BGR2LAB)
        m1, m2, red_mask = self._rgb_detect(hsv, lab, h1l, h1h, h2l, h2h,
                                            smin, smax, vmin, vmax)
        snap_rgb_hsv_mask = self._stamp_mode_tag(red_mask)
        ...
        snap_fg_rgb = None
        if self.use_bgsub_rgb.get():
            _rgb_src = self.bgsub_rgb_src.get()
            if _rgb_src == "auto":
                _rgb_src = self._bgsub_auto_rgb()
        ...
        if self.use_bgsub_ir.get():
            _ir_src = self.bgsub_ir_src.get()
            if _ir_src == "auto":
                # The IR side has no HSV mode — auto = the IR frame.
                _ir_src = "ir"
        ...
        def _yolo_box_adj(yst):
            """Per-step YOLO box (scale%, pad px) tuple, or None to
            fall back to the shared YOLO_Box_Scale / YOLO_Box_Pad.
            None is also returned when the step card never built the
            box vars, so old steps keep the shared behaviour."""
            try:
                return (int(yst["yolo_box_scale"].get()),
                        int(yst["yolo_box_pad"].get()))
            except Exception:
                return None

        def _run_yolo_for(src_name, class_ids=None, box_adj=None):
            ...
            or None) restricts which detections are kept. `box_adj` is
            an optional (scale%, pad px) override for the per-step box
            size. Both are folded into the cache key so per-step class
            filters / box sizes don't collide."""
            _ck = (src_name, class_ids, box_adj)
            ...
            try:
                ...
                for box in res.boxes:
                    ...
                    x1, y1, x2, y2 = self._yolo_box_adjust(
                        x1, y1, x2, y2,
                        base.shape[1], base.shape[0],
                        box_adj[0] if box_adj else None,
                        box_adj[1] if box_adj else None)
                    lbl = (f"{res.names[cid]} "
        ...
        def _apply_yolo_mode(running_mask, step_tuple):
            ...
            try:
                ...
                _, ymask, _ = _run_yolo_for(
                    src, _yolo_class_ids(yst), _yolo_box_adj(yst))
            ...
            for _up in self.user_pipelines:
                try:
                    ...
                    if _type == "rgb":
                        try:
                            ...
                            _bv  = cv2.inRange(_hsv_pre[:, :, 2],
                                               _up["v_lo"].get(),
                                               _up["v_hi"].get())
                            # LAB a*/b* masks — red<->green, yellow<->blue.
                            # a*/b* find a coloured object on any-
                            # brightness background (white/gray/black all
                            # sit near 128).
                            _lab_pre = cv2.cvtColor(_src_rgb_pre,
                                                    cv2.COLOR_BGR2LAB)
                            _ba = cv2.inRange(_lab_pre[:, :, 1],
                                              _up["a_lo"].get(),
                                              _up["a_hi"].get())
                            _bb = cv2.inRange(_lab_pre[:, :, 2],
                                              _up["b_lo"].get(),
                                              _up["b_hi"].get())
                            ...
                            view_lookup[f"up_{_nm}_a_mask"]  = _ba
                            view_lookup[f"up_{_nm}_b_mask"]  = _bb
                        except Exception:
                            _bh1 = _bh2 = _bs = _bv = _ba = _bb = None
                        ...
                        if ("V" in _ch_mode or _ch_mode == "full") and _bv is not None:
                            _to_and.append(_bv)
                        # LAB channels — lower-case so they never
                        # collide with the H/S/V letters; not part of
                        # "full" (which stays HSV-only).
                        if "a" in _ch_mode and _ba is not None:
                            _to_and.append(_ba)
                        if "b" in _ch_mode and _bb is not None:
                            _to_and.append(_bb)
                        if _to_and:
                            _det = _to_and[0]
                            for _m in _to_and[1:]:
                    ...
                    if _up.get("use_bgsub") and _up["use_bgsub"].get():
                        ...
                        if _bg_src == "auto":
                            _bg_src = self._bgsub_auto_branch(_up)
        ...
        # Re-run RGB / threshold detection on the post-pre-morph image
        # (overwrites the placeholder values computed near the top).
        hsv = cv2.cvtColor(_work_rgb_pre, cv2.COLOR_BGR2HSV)
        lab = cv2.cvtColor(_work_rgb_pre, cv2.COLOR_BGR2LAB)
        m1, m2, red_mask = self._rgb_detect(hsv, lab, h1l, h1h, h2l, h2h,
                                            smin, smax, vmin, vmax)
        snap_rgb_hsv_mask = self._stamp_mode_tag(red_mask)
        if (self.use_bgsub_rgb.get() and self.backSub_rgb
                and snap_fg_rgb is not None):
            red_mask = cv2.bitwise_and(red_mask, snap_fg_rgb)
        rgb_mask = red_mask.copy()
        snap_rgb_mask_pre = rgb_mask.copy()
        ...
        if self.use_yolo.get() and self.yolo_model:
            ...
            for box in res_rgb.boxes:
                ...
                x1, y1, x2, y2 = map(int, box.xyxy[0])
                x1, y1, x2, y2 = self._yolo_box_adjust(
                    x1, y1, x2, y2, f_rgb.shape[1], f_rgb.shape[0])
            ...
            for box in res_ir.boxes:
                ...
                x1, y1, x2, y2 = map(int, box.xyxy[0])
                x1, y1, x2, y2 = self._yolo_box_adjust(
                    x1, y1, x2, y2, ir_bgr.shape[1], ir_bgr.shape[0])
        ...
        # -- Combine view (A <op> B) --------------------------------------
        # 'combined' is ALWAYS a binary mask so it can feed other masks /
        # branches / classification, exactly like rgb_mask / ir_mask.
        # 'combined_overlay' is the colour composite (A in colour 1, B in
        # colour 2) for visualisation only. Op / A / B default to "none",
        # in which case the Combine tab produces nothing.
        try:
            from pipeline_ui_mixin import OVERLAY_COLORS
            ...
            if _cop in ("", "none") or _ca in ("", "none") \
                    or _cb in ("", "none"):
                # Nothing configured yet — leave both views blank.
                views["combined"]         = _blank
                views["combined_overlay"] = _blank
            else:
                _ima = views.get(_ca, _blank)
                _imb = views.get(_cb, _blank)
                _ga  = _to_gray(_ima)
                _gb  = cv2.resize(_to_gray(_imb),
                                  (_ga.shape[1], _ga.shape[0]))
                if _cop == "AND":
                    _comb = cv2.bitwise_and(_ga, _gb)
                elif _cop == "XOR":
                    _comb = cv2.bitwise_xor(_ga, _gb)
                else:   # "OR" and "OVERLAY" both union the two masks
                    _comb = cv2.bitwise_or(_ga, _gb)
                # binary result — usable as an input to other masks
                views["combined"] = cv2.cvtColor(_comb,
                                                 cv2.COLOR_GRAY2BGR)
                # colour composite — A in C1, B in C2, overlap blended
                _c1 = OVERLAY_COLORS.get(self.combine_c1_var.get(),
                                         (0, 0, 255))
                _c2 = OVERLAY_COLORS.get(self.combine_c2_var.get(),
                                         (0, 255, 0))
                _ov = np.zeros((_ga.shape[0], _ga.shape[1], 3), np.uint8)
                _am = _ga > 0
                _bm = _gb > 0
                _ov[_am] = _c1
                _ov[_bm] = _c2
                _ov[_am & _bm] = tuple((int(a) + int(b)) // 2
                                       for a, b in zip(_c1, _c2))
                views["combined_overlay"] = _ov
        except Exception:
            views["combined"]         = _blank
            views["combined_overlay"] = _blank
        ...
        if self.recording and self.writers:
            ...
            # Resize each panel once, reuse for both the per-panel
            # writers and the combined 2x3 mosaic.
            _rr = cv2.resize(f_rgb,           vsz)
            _rd = cv2.resize(rgb_det,         vsz)
            _rm = cv2.resize(rgb_md,          vsz)
            _ir = cv2.resize(ir_display_base, vsz)
            _id = cv2.resize(ir_det,          vsz)
            _im = cv2.resize(ir_md,           vsz)
            self.writers["rgb_raw"].write(_rr)
            self.writers["rgb_det"].write(_rd)
            self.writers["rgb_mask"].write(_rm)
            self.writers["ir_raw"].write(_ir)
            self.writers["ir_det"].write(_id)
            self.writers["ir_mask"].write(_im)
            _cw = self.writers.get("combined")
            if _cw is not None:
                _cw.write(cv2.vconcat([
                    cv2.hconcat([_rr, _rd, _rm]),
                    cv2.hconcat([_ir, _id, _im])]))
