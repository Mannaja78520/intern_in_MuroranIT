...
# Channel-combination modes for branch RGB detection. Upper-case
# H/S/V are HSV channels; lower-case a/b are LAB channels. A mode ANDs
# every channel whose letter it contains (H = H1 OR H2). "full" stays
# HSV-only for backward compatibility; "HSVab" is everything.
CHANNEL_MODES = ["H", "S", "V", "HS", "HV", "SV", "HSV",
                 "a", "b", "ab", "Ha", "HSVab", "full"]
...
class UserPipelinesMixin:
    ...
    def _all_view_names(self):
        ...
        # Built-in views — step entries sized from the LIVE pipeline
        # lengths (no fixed 20-step cap).
        nr = len(getattr(self, "rgb_pipeline", []))
        ni = len(getattr(self, "ir_pipeline", []))
        base = [
            "rgb_raw", "rgb_blur", "rgb_hsv_full", "rgb_hsv_H", "rgb_hsv_S",
            "rgb_hsv_V", "rgb_m1", "rgb_m2", "rgb_hsv_mask", "rgb_bgsub",
            "rgb_mask_pre",
            *[f"rgb_step{i}" for i in range(1, nr + 1)],
            "rgb_mask", "rgb_post_blur", "rgb_det",
            "ir_raw", "ir_gray", "ir_blur", "ir_clahe",
            "ir_bgsub", "ir_thresh", "ir_mask_pre",
            *[f"ir_step{i}" for i in range(1, ni + 1)],
    ...
    def _new_branch_record(self, name=None):
        ...
        return {
            ...
            # LAB a*/b* detection — used by the a / b / ab / HSVab
            # channel modes. a* is red<->green (>128 = red), b* is
            # yellow<->blue. Defaults target red on any background.
            "a_lo":     tk.IntVar(value=150),
            "a_hi":     tk.IntVar(value=255),
            "b_lo":     tk.IntVar(value=0),
            "b_hi":     tk.IntVar(value=255),
            ...
            "bgsub_src":      tk.StringVar(value="auto"),
    ...
    def _add_branch_pre_step(self, rec, step_dict, defer_ui=False):
        ...
        `defer_ui=True` skips the per-step UI rebuild — the caller
        (config load) rebuilds once after appending every step, which
        turns an O(n^2) load into O(n)."""
        ...
        if "pm_target" in step_dict:
            ...
            try:
                ...
                pass
        if not defer_ui and rec.get("pre_steps_row") is not None:
            self._rebuild_pre_morph_pipeline_ui(
                rec["pre_steps_row"], rec["pre_steps"])

    def _add_branch_step(self, rec, step_dict, defer_ui=False):
        """Append a single step to `rec`'s branch from a serialised
        step dict (the inverse of _step_to_dict).

        `defer_ui=True` skips the per-step UI rebuild so a config load
        can rebuild once at the end (O(n) instead of O(n^2))."""
        ...
        if not defer_ui and rec.get("steps_row") is not None:
    ...
    def _submit_new_branch(self, rec):
        ...
        rec["_committed_name"] = rec["name"].get().strip()
        self._build_user_pipeline_frame(rec)

    # ------------------------------------------------------------------
    # Dynamic branch rename — keep every reference wired
    # ------------------------------------------------------------------
    @staticmethod
    def _branch_view_rewrite(s, old, new):
        """Rewrite one view-name string from `up_<old>...` to
        `up_<new>...`. Anything else is returned untouched."""
        if not isinstance(s, str) or not old:
            return s
        if s == f"up_{old}":
            return f"up_{new}"
        if s.startswith(f"up_{old}_"):
            return f"up_{new}_" + s[len(f"up_{old}_"):]
        return s

    def _commit_branch_rename(self, rec):
        """Called when a branch's Name entry loses focus / Enter is
        pressed. If the name actually changed, rewrite EVERY view
        reference that pointed at the old `up_<old>...` name so the
        whole program stays wired automatically — branch sources,
        step combine-sources, YOLO / overlay source pickers and the
        main 6-panel selectors all follow the rename."""
        old = rec.get("_committed_name", "")
        new = rec["name"].get().strip()
        if not new:                       # empty name not allowed
            rec["name"].set(old)
            return
        if new == old:
            return
        # Reject a duplicate name — `up_<name>` must stay unique.
        for up in self.user_pipelines:
            if up is not rec and up["name"].get().strip() == new:
                rec["name"].set(old)
                return
        rec["_committed_name"] = new
        if not old:
            return
        rw = lambda s: self._branch_view_rewrite(s, old, new)
        # 1) every branch's source view
        for up in self.user_pipelines:
            up["source"].set(rw(up["source"].get()))
        # 2) every step's combine-source (10th tuple slot) in the RGB,
        #    IR and every branch step / pre-step pipeline
        pipelines = [getattr(self, "rgb_pipeline", []),
                     getattr(self, "ir_pipeline", [])]
        for up in self.user_pipelines:
            pipelines.append(up.get("steps") or [])
            pipelines.append(up.get("pre_steps") or [])
        for pl in pipelines:
            for st in pl:
                try:
                    st[9].set(rw(st[9].get()))
                except Exception:
                    pass
        # 3) per-step YOLO / overlay source pickers
        for st in (getattr(self, "_yolo_state", {}) or {}).values():
            for k in ("yolo_src", "yolo_mask_src"):
                if k in st:
                    st[k].set(rw(st[k].get()))
        for st in (getattr(self, "_overlay_state", {}) or {}).values():
            if "mask2_src" in st:
                st["mask2_src"].set(rw(st["mask2_src"].get()))
        # 4) main 6-panel view selectors
        for v in (getattr(self, "panel_view", {}) or {}).values():
            v.set(rw(v.get()))
        try:
            self._refresh()
        except Exception:
    ...
    def _open_branch_setup_dialog(self, rec, on_submit):
        ...
        for label, val, hint in [
            ("RGB / HSV / LAB detection",
             "rgb",
             "H1/H2 + S + V (HSV) + a*/b* (LAB), same as the main RGB "
             "pipeline. Pick which channels to AND together."),
            ("IR threshold",
             "ir",
        ...
        # -- RGB / HSV / LAB detection panel --------------------------
        rgb_lf = tk.LabelFrame(top, text="RGB / HSV / LAB detection",
                               font=("DejaVu Sans", 9, "bold"))
        ...
        ttk.Combobox(chrow, textvariable=rec["channels"],
                     ...
                     font=("DejaVu Sans", 9)).pack(side="left", padx=4)
        tk.Label(chrow,
                 text=("H=(H1 OR H2).  S, V, a, b each individual "
                       "(a/b = LAB).  Combo (e.g. HS, ab, HSVab) = "
                       "AND of those.  full = H AND S AND V."),
                 font=("DejaVu Sans", 9), fg="#c8c8c8",
        ...
        _h1_r = _h_row(rgb_lf, "H1",   rec["h1_lo"], rec["h1_hi"], H_MAX)
        _h2_r = _h_row(rgb_lf, "H2",   rec["h2_lo"], rec["h2_hi"], H_MAX)
        _s_r  = _h_row(rgb_lf, "S",    rec["s_lo"],  rec["s_hi"],  255)
        _v_r  = _h_row(rgb_lf, "V",    rec["v_lo"],  rec["v_hi"],  255)
        _a_r  = _h_row(rgb_lf, "LAB a*", rec["a_lo"], rec["a_hi"], 255)
        _b_r  = _h_row(rgb_lf, "LAB b*", rec["b_lo"], rec["b_hi"], 255)

        # Show only the channel rows the Channels mode actually uses
        # (mode "H" -> just H1/H2, "ab" -> just LAB a*/b*, etc.), so the
        # panel never shows sliders that have no effect. Forget-all
        # then re-pack keeps the H1/H2/S/V/a/b order stable.
        def _refresh_branch_channels(*_):
            mode = rec["channels"].get() or "HSV"
            order = [
                (_h1_r, ("H" in mode) or mode == "full"),
                (_h2_r, ("H" in mode) or mode == "full"),
                (_s_r,  ("S" in mode) or mode == "full"),
                (_v_r,  ("V" in mode) or mode == "full"),
                (_a_r,  ("a" in mode)),
                (_b_r,  ("b" in mode)),
            ]
            for r, _ in order:
            ...
            for r, show in order:
                if show:
                    r.pack(fill="x", padx=6, pady=1)
        rec["channels"].trace_add("write", _refresh_branch_channels)
    ...
    def _build_user_pipeline_frame(self, rec):
        ...
        tk.Label(hdr, text="Name:",
                 font=("DejaVu Sans", 8)).pack(side="left")
        _name_entry = tk.Entry(hdr, textvariable=rec["name"], width=12,
                               font=("DejaVu Sans", 8))
        _name_entry.pack(side="left", padx=2)
        # Commit the rename when the user leaves the field or hits
        # Enter — this rewrites every reference to the branch so the
        # whole program stays wired (dynamic rename).
        _name_entry.bind("<FocusOut>",
                         lambda e, _r=rec: self._commit_branch_rename(_r))
        _name_entry.bind("<Return>",
                         lambda e, _r=rec: self._commit_branch_rename(_r))
        ...
        tk.Checkbutton(opt_row, text="Use BG sub",
                       ...
                       font=("DejaVu Sans", 8)).pack(side="left")
        # BG-sub channel: "auto" follows the branch's detection mode
        # (IR branch -> IR frame; RGB branch -> the channel its CHANNELS
        # mode relies on). "rgb"/"ir" use the full BGR frame, while a
        # single HSV channel ("H"/"S"/"V") or LAB channel ("L"/"a"/"b")
        # feeds JUST that channel into MOG2 — e.g. subtract saturation
        # or a* drift without colour / brightness noise interfering.
        ...
        a_lo_r  = _slider_row(hsv_frame, "a* min", rec["a_lo"])
        a_hi_r  = _slider_row(hsv_frame, "a* max", rec["a_hi"])
        b_lo_r  = _slider_row(hsv_frame, "b* min", rec["b_lo"])
        b_hi_r  = _slider_row(hsv_frame, "b* max", rec["b_hi"])

        h_rows = (h1_lo_r, h1_hi_r, h2_lo_r, h2_hi_r)
        s_rows = (s_lo_r, s_hi_r)
        v_rows = (v_lo_r, v_hi_r)
        a_rows = (a_lo_r, a_hi_r)
        b_rows = (b_lo_r, b_hi_r)

        def _refresh_channel_visibility(*_):
            mode = rec["channels"].get() or "HSV"
            groups = (
                (h_rows, ("H" in mode) or mode == "full"),
                (s_rows, ("S" in mode) or mode == "full"),
                (v_rows, ("V" in mode) or mode == "full"),
                (a_rows, ("a" in mode)),          # LAB channels (lower-case)
                (b_rows, ("b" in mode)),
            )
            # Forget all rows, then re-pack the visible ones in order so
            # the H/S/V/a/b layout stays stable when modes are switched.
            for rows, _show in groups:
                for r in rows:
                    r.pack_forget()
            for rows, show in groups:
                if show:
                    for r in rows:
                        r.pack(fill="x", padx=4, pady=0)
