...
def main():
    ...
    A) INSIDE the analyzer GUI: add a step to the main RGB pipeline,
       set Combine = OR, source = up_maxrgb. Then the OR is baked in
    ...
    masks  = detect(bgr)
    branch = masks.get("up_maxrgb")
    if branch is None:
        print("example 5: no branch 'up_maxrgb' in this export - "
              "add one in the analyzer and re-export.")
        return
    merged = cv2.bitwise_or(masks["rgb_mask"], branch)   # <-- the OR
    cv2.imwrite("ex5_merged.png", merged)
    print("example 5: wrote ex5_merged.png  (rgb_mask OR up_maxrgb)")
