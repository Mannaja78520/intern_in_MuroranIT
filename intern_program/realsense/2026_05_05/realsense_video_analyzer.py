...
class VideoAnalyzer(PipelineUIMixin, FlowchartMixin, UserPipelinesMixin):
    ...
    def _build_ui(self):
        ...
        tk.Button(ctrl, text="Apply", width=7,
    ...
    def _process(self, f_rgb, f_ir):
        ...
        def _run_step(running, snap_steps, op_var, n_var, kx_var, ky_var,
            ...
            if comb_en_var.get():
                ref    = _resolve_src(snap_steps, comb_src_var.get(), mask_pre)
                branch = _apply_op(ref, op_var.get(),
                ...
                if branch_sink is not None and branch_key is not None:
                    branch_sink[branch_key] = branch.copy()
                cop = comb_op_var.get()
                if cop == "AND":   return cv2.bitwise_and(running, branch)
                elif cop == "OR":  return cv2.bitwise_or(running,  branch)
                else:              return cv2.bitwise_xor(running,  branch)
        ...
        def _run_user_pipelines():
            for _up in self.user_pipelines:
                try:
                    ...
                    # H1, H2, S, V, IR each have an enable flag. Enabled
                    # channels are AND'd together (H1+H2 OR'd inside
                    # first), producing up_<name>_det.
                    try:
                    ...
                    try:
                        _bir = cv2.inRange(snap_ir_gray,
                                           _up["ir_lo"].get(),
                                           _up["ir_hi"].get())
                        # Resize IR mask to match RGB shape so the AND
                        # cross-channel works regardless of camera res.
                        if (_bir is not None
                                and _bir.shape != rgb_mask.shape):
                    ...
                    # Combine selected channels per the rules:
                    #   H_combined = (H1 if en) OR (H2 if en)
                    #   det        = AND of [H_combined, S, V, IR]
                    #                taking only channels whose enable
                    #                flag is set.
                    _h_comb = None
                    if _up["en_h1"].get() and _bh1 is not None:
                        _h_comb = _bh1
                    if _up["en_h2"].get() and _bh2 is not None:
                        _h_comb = (_bh2 if _h_comb is None
                                   else cv2.bitwise_or(_h_comb, _bh2))
                    _to_and = []
                    if _h_comb is not None:
                        _to_and.append(_h_comb)
                    if _up["en_s"].get()  and _bs  is not None:
                        _to_and.append(_bs)
                    if _up["en_v"].get()  and _bv  is not None:
                        _to_and.append(_bv)
                    if _up["en_ir"].get() and _bir is not None:
                        _to_and.append(_bir)
                    if _to_and:
                        _det = _to_and[0]
                        for _m in _to_and[1:]:
                        ...
                        # No channels enabled → empty mask.
                        _det = np.zeros_like(rgb_mask)
                    view_lookup[f"up_{_nm}_det"] = _det

                    # Resolve source image: prefer view_lookup (raw mask),
                    # else fall back to a blank.
        ...
        rgb_branch_imgs = {}
        snap_rgb_steps = []
        for _idx, (en_var, op_var, n_var, dir_var, kx_var, ky_var, thresh_var,
                comb_en_var, comb_op_var, comb_src_var) in enumerate(self.rgb_pipeline):
            if en_var.get():
        ...
        snap_ir_steps = []
        for _idx, (en_var, op_var, n_var, dir_var, kx_var, ky_var, thresh_var,
                comb_en_var, comb_op_var, comb_src_var) in enumerate(self.ir_pipeline):
            if en_var.get():
        ...
        for up in self.user_pipelines:
            ...
            if _final is None:
                continue
            views[f"up_{_nm}"] = _mask_bgr(_final, _clr)
        ...
        if self.all_masks_win and self.all_masks_win.winfo_exists():
            ...
            for view_name, lbl in self.all_masks_labels.items():
                # Combine-step sub-thumbnails are keyed as
                #     "<step>__img1@<view>"  or  "<step>__img2@<view>"
                # so we look up the resolved view after the "@".
                if "@" in view_name:
            ...
            for _i, (_en, _op, _n, _d, _kx, _ky, _t, _cen, _cop, _csr) in enumerate(self.rgb_pipeline):
                ...
                if _k in self.all_masks_step_labels:
                    _base = _op.get() if _en.get() else "—"
                    _suf  = f" ⊕{_cop.get()}({_csr.get()})" if (_en.get() and _cen.get()) else ""
                    self.all_masks_step_labels[_k].config(text=f"S{_i+1}: {_base}{_suf}")
            for _i, (_en, _op, _n, _d, _kx, _ky, _t, _cen, _cop, _csr) in enumerate(self.ir_pipeline):
                ...
                if _k in self.all_masks_step_labels:
                    ...
                    _suf  = f" ⊕{_cop.get()}({_csr.get()})" if (_en.get() and _cen.get()) else ""
                    self.all_masks_step_labels[_k].config(text=f"S{_i+1}: {_base}{_suf}")
            # User-pipeline step labels — same live-refresh treatment.
            for _up in self.user_pipelines:
                _nm = _up["name"].get().strip() or "branch"
                for _i, (_en, _op, _n, _d, _kx, _ky, _t,
                    ...
                    if _k in self.all_masks_step_labels:
                        _base = _op.get() if _en.get() else "—"
                        _suf  = (f" ⊕{_cop.get()}({_csr.get()})"
                                 if (_en.get() and _cen.get()) else "")
                        self.all_masks_step_labels[_k].config(
                            text=f"S{_i+1}: {_base}{_suf}")
