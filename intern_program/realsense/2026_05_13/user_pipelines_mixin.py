...
class UserPipelinesMixin:
    ...
    def _new_branch_record(self, name=None):
        ...
        return {
            ...
            # Pre-detect blur. type selects Gaussian / Median /
            # Bilateral (matches the Ops list). > 0 K blurs the source
            # image (rgb_raw / ir_gray) BEFORE HSV / IR thresholding,
            # so the binary mask is cleaner without burning blur into
            # downstream Morph ops. KY is the Sigma for Bilateral
            # (ignored by Gaussian / Median).
            "pre_blur_type":  tk.StringVar(value="GaussBlur"),
            "pre_blur_k":     tk.IntVar(value=0),
            "pre_blur_ky":    tk.IntVar(value=0),
    ...
    def _submit_new_branch(self, rec):
        ...
        for _key in ("type", "channels",
                     ...
                     "s_lo",  "s_hi",  "v_lo",  "v_hi",
                     "ir_lo", "ir_hi",
                     "pre_blur_type", "pre_blur_k", "pre_blur_ky",
                     "use_bgsub", "bgsub_src",
    ...
    def _build_user_pipeline_frame(self, rec):
        ...
        # Pre-detect blur (shared between rgb / ir types). Type +
        # params match the Ops list:
        #   GaussBlur / MedianBlur -> K (odd kernel; 0 = off)
        #   BilateralBlur          -> K = diameter, KY = sigma
        pre_blur_type_row = tk.Frame(det_inline)
        pre_blur_type_row.pack(fill="x", padx=4, pady=(2, 0))
        tk.Label(pre_blur_type_row, text="Pre-blur:",
                 font=("Arial", 8, "bold"), fg="#aaff88",
                 width=10, anchor="w").pack(side="left")
        ttk.Combobox(pre_blur_type_row, textvariable=rec["pre_blur_type"],
                     values=["None", "GaussBlur", "MedianBlur", "BilateralBlur"],
                     width=12, state="readonly",
                     font=("Arial", 8)).pack(side="left", padx=2)

        pre_blur_row = tk.Frame(det_inline)
        pre_blur_row.pack(fill="x", padx=4, pady=0)
        tk.Label(pre_blur_row, text=" K:",
                 font=("Arial", 8), width=10,
                 anchor="w").pack(side="left")
        tk.Spinbox(pre_blur_row, textvariable=rec["pre_blur_k"],
                   from_=0, to=51, width=5,
                   font=("Arial", 8)).pack(side="left", padx=2)
        tk.Scale(pre_blur_row, variable=rec["pre_blur_k"],
                 from_=0, to=51, orient="horizontal",
                 length=140, showvalue=False).pack(side="left", padx=2)

        pre_blur_row_ky = tk.Frame(det_inline)
        pre_blur_row_ky.pack(fill="x", padx=4, pady=(0, 2))
        tk.Label(pre_blur_row_ky, text=" Sigma:",
                 font=("Arial", 8), width=10,
                 anchor="w").pack(side="left")
        tk.Spinbox(pre_blur_row_ky, textvariable=rec["pre_blur_ky"],
                   from_=0, to=200, width=5,
                   font=("Arial", 8)).pack(side="left", padx=2)
        tk.Scale(pre_blur_row_ky, variable=rec["pre_blur_ky"],
                 from_=0, to=200, orient="horizontal",
                 length=140, showvalue=False).pack(side="left", padx=2)

        def _refresh_pre_blur_visibility(*_):
            # Sigma row only matters for Bilateral; hide it otherwise
            # so the user doesn't tweak an ignored value. Use
            # `after=pre_blur_row` so when re-shown it sits right
            # below K, not at the bottom of the panel.
            t = rec["pre_blur_type"].get()
            if t == "None":
                pre_blur_row.pack_forget()
                pre_blur_row_ky.pack_forget()
                return
            pre_blur_row.pack(fill="x", padx=4, pady=0,
                              after=pre_blur_type_row)
            if t == "BilateralBlur":
                pre_blur_row_ky.pack(fill="x", padx=4, pady=(0, 2),
                                     after=pre_blur_row)
            else:
                pre_blur_row_ky.pack_forget()
        rec["pre_blur_type"].trace_add("write",
                                       _refresh_pre_blur_visibility)
        _refresh_pre_blur_visibility()
    ...
    def _branch_summary(self, rec):
        ...
        pre_t = "GaussBlur"
        try:
            pre_k = int(rec.get("pre_blur_k", tk.IntVar(value=0)).get())
            pre_ky = int(rec.get("pre_blur_ky", tk.IntVar(value=0)).get())
            pre_t  = rec.get("pre_blur_type",
                             tk.StringVar(value="GaussBlur")).get()
        except Exception:
            pass
        if pre_k > 0:
            pb = (f"  pre-blur {pre_t} K={pre_k}"
                  + (f" Sig={pre_ky}" if pre_t == "BilateralBlur" else ""))
        else:
            pb = ""
        suf = pb + bg + ov
        if t == "rgb":
