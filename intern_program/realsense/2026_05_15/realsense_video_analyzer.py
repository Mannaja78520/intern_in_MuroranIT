...
class VideoAnalyzer(PipelineUIMixin, FlowchartMixin, UserPipelinesMixin):
    def __init__(self, root):
        ...
        # If the saved theme is light, remap every widget the build
        # just created (build sites use dark-tuned literals).
        if self.ui_theme != "dark":
            self._retheme_tree(self.root)
    ...
    def _build_ui(self):
        ...
        _hdr(shared_tab, "Appearance", "#aaccff")
        _theme_row = tk.Frame(shared_tab)
        _theme_row.pack(fill="x", pady=2)
        tk.Label(_theme_row, text="Theme:",
                 font=("DejaVu Sans", 9)).pack(side="left")
        self.ui_theme_var = tk.StringVar(value=self.ui_theme)
        ttk.Combobox(_theme_row, textvariable=self.ui_theme_var,
        ...
        tk.Label(_theme_row, text="(applies instantly)",
                 font=("DejaVu Sans", 8, "italic"),
                 fg="#c8c8c8").pack(side="left", padx=4)
    ...
    # ------------------------------------------------------------------
    def _retheme_one(self, w):
        """Remap a single widget's recognised colours to the current
        theme. Only hexes registered in THEME_COLORS are touched, so
        mask colours / black thumbnails / anything custom is left
        alone. Works in both directions (dark<->light)."""
    ...
    def _retheme_tree(self, widget):
        """Recursively retheme `widget` and its descendants. Separate
        Toplevels (All-Masks, zoom windows) are skipped — they keep
        their own look."""
        try:
            self._retheme_one(widget)
        except Exception:
            pass
        try:
        ...
        for ch in children:
            if isinstance(ch, tk.Toplevel):
                continue
            self._retheme_tree(ch)

    def _on_theme_change(self, *_a):
        """Apply a live theme switch: re-run option_add / ttk styles
        (so future widgets use the new palette) and walk the existing
        widget tree remapping recognised colours."""
