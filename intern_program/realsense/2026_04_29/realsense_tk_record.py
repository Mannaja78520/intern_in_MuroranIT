...
class RealSenseDashboardRecorder:
    def __init__(self, window, window_title):
        ...
        try:
            ...
            self.has_imu = True
            print("IMU enabled (D435i detected)")
        except Exception:
            # Device has no IMU (e.g. D435) — restart without motion streams
            self.pipeline = rs.pipeline()
            self.config = rs.config()
            self.config.enable_stream(rs.stream.color, 640, 480, rs.format.bgr8, 30)
            self.config.enable_stream(rs.stream.infrared, 1, 640, 480, rs.format.y8, 30)
            try:
                profile = self.pipeline.start(self.config)
                print("IMU not available on this device — orientation will not be saved")
    ...
    def _save_orientation(self, session_dir):
        if not self.has_imu or self.latest_accel is None:
            return
        ax, ay, az = self.latest_accel.x, self.latest_accel.y, self.latest_accel.z
        pitch = math.degrees(math.atan2(ax, math.sqrt(ay ** 2 + az ** 2)))
        roll  = math.degrees(math.atan2(ay, az))
        data = {
            "timestamp": datetime.now().isoformat(),
            "accel": {"x": ax, "y": ay, "z": az},
            "gyro":  {"x": self.latest_gyro.x, "y": self.latest_gyro.y, "z": self.latest_gyro.z}
                     if self.latest_gyro else None,
            "orientation_deg": {"pitch": round(pitch, 3), "roll": round(roll, 3)},
        }
        path = os.path.join(session_dir, "orientation.json")
        with open(path, "w") as f:
            json.dump(data, f, indent=2)
        print(f"Orientation saved → pitch={pitch:.1f}° roll={roll:.1f}°")
    ...
    def start_recording(self):
        ...
        self._save_orientation(session_dir)
