        ...
        for up in user_pipes:
            ...
            try:
                ...
                try:
                    ...
                    _pre_k = 0
                col_cursor = 1
                # Source-image pipeline: raw frame the branch sees,
                # then the pre-blurred frame when pre-blur is active.
                # Shown for rgb/ir branches so the user can trace
                # "what image actually fed the detection".
                if _btype in ("rgb", "ir"):
                    _info_nodes.append(
                        (f"_info_{_nm}_src_raw",
                         col_cursor, 0, "src raw"))
                    col_cursor += 1
                    if _pre_k > 0:
                        _info_nodes.append(
                            (f"_info_{_nm}_src_blur",
                             col_cursor, 0, "src blur"))
                        col_cursor += 1
                    # Source AFTER bg subtraction — only when bgsub
                    # is enabled on this branch.
                    if _show_bg:
                        _info_nodes.append(
                            (f"_info_{_nm}_src_bgsub",
                             col_cursor, 0, "src bg-sub"))
                        col_cursor += 1
                if _btype == "rgb":
                ...
                if _show_bg:
                    ...
                    col_cursor += 1
                # Combined detection — when channels mix (HSV / HS /
                # HV / SV / full / multi-channel modes), the per-
                # channel masks above don't show what the combined
                # AND-ed detection looks like. Add a dedicated
                # branch_det thumbnail so the user can see the
                # actual `up_<name>_det` view that drives the
                # pipeline's first step.
                _info_nodes.append(
                    (f"_info_{_nm}_det",
                     col_cursor, 0, "branch_det"))
                col_cursor += 1
            except Exception:
                _info_nodes = []
