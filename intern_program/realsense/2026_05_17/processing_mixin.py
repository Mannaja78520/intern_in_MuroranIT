...
class ProcessingMixin:
    ...
    def _lab_mask(self, lab):
        """LAB a*/b* threshold mask. a* is red<->green, b* is
        yellow<->blue (both 128 = neutral). Reads A_min/A_max/B_min/
        B_max from the detection sliders. White / gray / black sit at
        a*=b*=128, so banding a*/b* away from 128 isolates a coloured
        object on any-brightness background."""
        try:
            amn = self.sv["A_min"].get(); amx = self.sv["A_max"].get()
            bmn = self.sv["B_min"].get(); bmx = self.sv["B_max"].get()
        except Exception:
            amn, amx, bmn, bmx = 150, 255, 0, 255
    ...
    def _rgb_detect(self, hsv, lab, h1l, h1h, h2l, h2h,
        ...
        Returns (m1, m2, mask):
          ...
          value   - V band only.
          lab     - m1 is the LAB a*/b* mask, m2 empty.
