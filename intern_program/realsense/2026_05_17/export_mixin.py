...
# ======================================================================
#  Standalone runtime — copied verbatim into every exported script.
...
class ExportMixin:
    """Adds the 'Export pipeline' feature: standalone .py + .png."""

    # ------------------------------------------------------------------
    def _export_pipeline(self):
        """Ask for a path, then write:
           <name>.py            - the self-contained pipeline
           <name>.png           - flowchart + settings report
           <name>_examples/     - folder with 7 standalone examples"""
        from tkinter import filedialog
        path = filedialog.asksaveasfilename(
            title="Export pipeline (.py + .png + examples folder)",
            defaultextension=".py",
        ...
        try:
            ...
            self.lbl_status.config(
                text="Exported: %s + %s + %s/ (%d examples)" % (
    ...
    def _settings_report(self, cfg):
        ...
        L.append("  LAB   a* %s..%s     b* %s..%s   (lab / hsv_lab modes)"
    ...
    def _export_build_png(self, cfg, png_path):
        ...
        for title, boxes, colour in rows:
            draw.text((margin, y + bh // 2 - 6), title, fill=(230, 230, 230))
            x = margin + title_w
