...
class UserPipelinesMixin:
    ...
    def _open_branch_setup_dialog(self, rec, on_submit):
        ...
        _h_row(rgb_lf, "LAB a*", rec["a_lo"], rec["a_hi"], 255)
        _h_row(rgb_lf, "LAB b*", rec["b_lo"], rec["b_hi"], 255)
    ...
    def _build_user_pipeline_frame(self, rec):
        ...
        def _refresh_channel_visibility(*_):
            ...
            show_a = ("a" in mode)          # LAB channels (lower-case)
            show_b = ("b" in mode)
            for rows, show in ((h_rows, show_h), (s_rows, show_s),
                               (v_rows, show_v), (a_rows, show_a),
                               (b_rows, show_b)):
                for r in rows:
                    if show:
                        r.pack(fill="x", padx=4, pady=0)
